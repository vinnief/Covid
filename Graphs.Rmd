...
verbose <- 1
theLog <- file( "Graphs.log", open = "wt")
sink(theLog, type = c("output"))
source("loadData.R")
sink()
```
...
```{r graphs walkthrough, include=FALSE, echo=FALSE}
#verbose <- 4
...
if ( weekdays( Sys.Date() , abbreviate = FALSE) == "Thursday") { 
  ...
  tim = Sys.time()
  walkThrough(lpdf = ECDC,regions = ECDC.Regios, graphlist = myGraphNrs,  smoothn=1, myFolderDate  = 'current', ordre = 'RG') 
...
verbose = 1
makeHistoryGraphs(JHH, regions = JHH.Regios, fromDates = seq(as.Date('2020-01-01'),Sys.Date(),  by = '1 month'), ordre = 'GR')  
