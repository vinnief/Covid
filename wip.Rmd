...
```
```{r}
Belg <- ECDC[ECDC$Country.Region=="Belgium",] 
View(Belg)


```
### during browse in graphit:
check why graph5ardcra_fyil crashes on JHH.Regios[[12]]
```{r }

lpdf %>% group_by(mygroup, PSCR) %>% summarize(howmany =  n())%>% View
verbose <- 7 
walkThrough( lpdf = JHH, regions = JHH.Regios, graphlist = "graph6Dardcra_fiyl", 
             myFolderDate  = 'current', logy = T, ordre = 'RG') #crashes on recovered passengers in World12 with T, works with False. 20201209
```
...
graphDanny(JHH, JHH.Regios$`JHH World2`) #to be deprecated
graph1aa_finl(ECDC, ECDC.Regios$`ECDC World2`,labmeth = "dl_top.points")
graph1aa_finl(ECDC, ECDC.Regios$`ECDC World1`, smoothn = 7,labmeth = "dl_last.points", putlegend = FALSE)
graph1aa_finl(JHH, JHH.Regios$`JHH World2`, smoothn = 7,labmeth = "dl_last.points", putlegend = FALSE)
