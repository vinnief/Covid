...
if (!exists('JHH')) source('loadData.R') else 
  if (max(JHH$Date)< Sys.Date()-1) source('loadData.R') else
    source('definitions.R')
 #do this as little as needed. at least once, before the chunks below!
```
...
```{r save graphs, eval=FALSE, message=FALSE, include=FALSE}
