...
graphit <- function(lpti, countries, minVal  = 1, ID  = "PSCR", xvar  = "Date", 
                    ...
                    myFolderDate  = 'random', myFolderType  = "", savename  = "", putlegend = TRUE, 
                    size = 2, returnID  = "PSCR", area  = FALSE, position  = 'stack', facet  = FALSE, 
                    sorted  = TRUE, labmeth = 'dl_points', from = '2019-12-01', to  = Sys.Date()){
  ...
  lines_only <- lpdf %>% select(!!ID,mygroup) %>% group_by_at(c(1,2)) %>% 
    filter(n() > 1) 
  if (verbose >= 6) {view(lines_only)}
  lines_only <- lines_only %>% unique()
  if (verbose >= 6) {view('country-lines with 2+ datapoints:') ; print(lines_only)}
